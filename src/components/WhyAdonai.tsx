          ...
          <div className="lg:col-span-7 space-y-6">
            {points.map((point, index) => {
              ...
              return (
                <div
                  key={index}
                  className="adonai-card p-6 sm:p-7 rounded-2xl bg-slate-50/70 border border-slate-200 hover:border-[#2E7D32]/40 transition-all flex flex-col sm:flex-row items-start gap-5 group"
                >
                  <div className="w-13 h-13 rounded-2xl bg-[#F1F7F2] border border-[#2E7D32]/20 flex items-center justify-center text-[#2E7D32] shrink-0 group-hover:bg-[#2E7D32] group-hover:text-[#2E7D32] transition-colors shadow-2xs">
                    <Icon className="w-6 h-6" />
                  </div>

                  <div className="space-y-1.5 flex-1">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <h3 className="font-display font-black text-2xl text-[#1B4D2E] uppercase tracking-tight">
                        {point.title}
                      </h3>
                      <span className="text-[10px] uppercase tracking-wider font-extrabold text-[#2E7D32] bg-white border border-slate-200 px-2.5 py-0.5 rounded-full">
                        {point.badge}
                      </span>
                    </div>
          ...
          <div className="lg:col-span-5 flex flex-col justify-center">
            <div className="relative rounded-3xl overflow-hidden shadow-2xl border-4 border-white bg-slate-900 group">
              ...
              <div className="absolute top-5 left-5 bg-white/95 backdrop-blur-md py-2 px-3.5 rounded-xl border border-slate-200 shadow-md flex items-center gap-2.5">
                ...
                <span className="text-xs font-bold text-[#1B4D2E]">100% Pratique en atelier</span>
              </div>

              {/* Cartouche d'information en bas de l'image */}
              <div className="absolute bottom-6 left-6 right-6 p-5 rounded-2xl bg-white/95 backdrop-blur-md border border-white/40 shadow-xl space-y-2 text-[#1B4D2E]">
